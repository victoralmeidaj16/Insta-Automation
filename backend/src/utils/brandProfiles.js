...
// Curated copy rules are deliberately replaceable: an account must be able to
// remove a preset rule instead of inheriting it forever.
function useProfileRules(profileValue, presetValue) {
    return Array.isArray(profileValue) && profileValue.length > 0
        ? profileValue
        : (Array.isArray(presetValue) ? presetValue : []);
}

function mergeNarrativeStructure(profileValue = {}, presetValue = {}) {
    const profile = profileValue && typeof profileValue === 'object' ? profileValue : {};
    const preset = presetValue && typeof presetValue === 'object' ? presetValue : {};

    return {
        ...preset,
        ...profile,
        slideRoles: useProfileRules(profile.slideRoles, preset.slideRoles)
    };
}

function mergeVisualIdentity(profileValue = {}, presetValue = {}) {
    const profile = profileValue && typeof profileValue === 'object' ? profileValue : {};
    const preset = presetValue && typeof presetValue === 'object' ? presetValue : {};
    ...
    return {
        ...preset,
        ...profile,
        imagePromptGuidelines: useProfileRules(profile.imagePromptGuidelines, preset.imagePromptGuidelines)
    };
}
...
export function normalizeBrandKey(input = {}) {
    ...
    if (normalized.includes('elevepic')) return 'elevepic';
    ...
    if (brandKey === 'inner-boost') {
        return {
            ...
            editorialPillars: [
                {
                    id: 'choque',
                    name: 'Choque — O Despertar',
                    description: 'Identificação com ansiedade, caos mental e o problema nu e cru',
                    weight: 25,
                    formats: ['carousel-premium', 'static'],
                    captionStyle: 'Impactante, identificação emocional direta, sem solução ainda. CTA: salvar.',
                    enabled: true
                },
                {
                    id: 'clareza',
                    name: 'Clareza — A Virada',
                    description: 'Dicas práticas de organização mental, ferramentas e métodos de produtividade',
                    weight: 35,
                    formats: ['carousel-html', 'carousel-premium'],
                    captionStyle: 'Útil, prático, passo a passo. CTA: salvar e aplicar.',
                    enabled: true
                },
                {
                    id: 'saida',
                    name: 'Saída — A Solução',
                    description: 'Produto, método concreto e o novo eu possível',
                    weight: 25,
                    formats: ['carousel-premium', 'story'],
                    captionStyle: 'Solução clara, direto ao ponto. CTA: conhecer o produto.',
                    enabled: true
                },
                {
                    id: 'identidade',
                    name: 'Identidade de Marca',
                    description: 'Conteúdo motivacional de transformação e pertencimento',
                    weight: 15,
                    formats: ['static', 'story'],
                    captionStyle: 'Motivacional, pertencimento, identidade. CTA: seguir.',
                    enabled: true
                }
            ],
            contentSchedule: {
            ...
            brandKit: {
                ...
                voice: 'Introspectiva, direta e honesta. Nomeie o caos mental sem dramatização vazia e conduza para uma clareza possível.',
                toneRules: [
                    'Fale com profundidade e precisão, sem tom motivacional genérico.',
                    'Transforme desconforto emocional em um próximo passo concreto.',
                    'Evite promessas fáceis ou positividade forçada.'
                ],
                copyArchetypes: [
                    'ESPELHO: "Você chama de preguiça o que já virou sobrecarga."',
                    'VIRADA: "Clareza não começa quando tudo está resolvido."'
                ],
                headlineExamples: ['O CAOS NÃO É FALTA DE VONTADE', 'SUA MENTE PEDE ESTRUTURA'],
                visualIdentity: {
                    photographyStyle: 'Editorial dark, psicológico e humano, com contraste alto e textura real.',
                    colorUsage: 'Preto #0B0B0D dominante; Neon Blue #00C2FF e Neon Green #00F5A0 apenas como acentos.',
                    typographyFeel: 'Minimalista, tecnológico e intenso.',
                    imagePromptGuidelines: [
                        'Usar metáforas visuais de clareza, reflexão e sobrecarga.',
                        'Evitar cores quentes e cenários alegres ou publicitários.',
                        'Não usar texto legível nas imagens.'
                    ]
                },
                carouselTemplate: {
    ...
    if (brandKey === 'viver-mais') {
        return {
            ...
            editorialPillars: [
                {
                    id: 'bem-estar',
                    name: 'Bem-Estar e Longevidade',
                    description: 'Dicas de saúde, hábitos saudáveis e qualidade de vida',
                    weight: 35,
                    formats: ['carousel', 'static'],
                    captionStyle: 'Informativo, calmante, CTA de salvar.',
                    enabled: true
                },
                {
                    id: 'produto',
                    name: 'Produto e Solução',
                    description: 'Apresentação de produtos, benefícios e diferenciais',
                    weight: 30,
                    formats: ['static', 'carousel'],
                    captionStyle: 'Direto, benefício claro, CTA de conhecer.',
                    enabled: true
                },
                {
                    id: 'educacao',
                    name: 'Educação em Saúde',
                    description: 'Conteúdo educativo sobre saúde, nutrição e suplementação',
                    weight: 25,
                    formats: ['carousel'],
                    captionStyle: 'Didático, baseado em ciência, CTA de salvar.',
                    enabled: true
                },
                {
                    id: 'motivacao',
                    name: 'Motivação e Estilo de Vida',
                    description: 'Inspiração, comunidade e identidade de marca',
                    weight: 10,
                    formats: ['static', 'story'],
                    captionStyle: 'Inspiracional, pertencimento, CTA de seguir.',
                    enabled: true
                }
            ],
            contentSchedule: {
    ...
    if (brandKey === 'tudy') {
        return {
            ...
            editorialPillars: [
                {
                    id: 'pratica-ativa',
                    name: 'Prática Ativa > Leitura Passiva',
                    description: 'Quiz, flashcards e método ativo vs. reler passivo — por que reler não fixa',
                    weight: 30,
                    formats: ['carousel-html', 'static'],
                    captionStyle: 'ESTRUTURA: 1) Identifique com a dor (ex: "você relê e esquece tudo"). 2) Ensine por que reler não funciona — dado ou ciência. 3) Apresente a técnica correta (prática ativa, quiz, flashcards). 4) Mencione o Tudy APENAS no CTA final como ferramenta que aplica isso. CTA: "Salva esse post" ou "Manda pra quem estuda muito e aprende pouco".',
                    enabled: true
                },
                {
                    id: 'ia-organiza',
                    name: 'IA Que Organiza Seu Estudo',
                    description: 'Demonstrações reais: foto → estudo pronto, PDF → trilha, pergunta → resolução',
                    weight: 35,
                    formats: ['carousel-premium', 'story'],
                    captionStyle: 'ESTRUTURA: 1) Abra com a situação (travou em exercício? não sabe por onde começar?). 2) Mostre o caminho passo a passo — o raciocínio, o método. 3) Cite o Tudy como a ferramenta que faz isso automaticamente, apenas no fim. CTA: "Comenta o assunto que você trava" ou "Salva para usar na próxima prova".',
                    enabled: true
                },
                {
                    id: 'progresso-visivel',
                    name: 'Progresso Visível',
                    description: 'Streak, evolução em provas, resultados reais de usuários',
                    weight: 20,
                    formats: ['static', 'story'],
                    captionStyle: 'ESTRUTURA: 1) Enuncie a técnica ou método de forma direta (ex: "A técnica que muda sua retenção"). 2) Explique por que funciona — dado, ciência ou lógica clara. 3) Dê instruções práticas aplicáveis HOJE sem o app. 4) Mencione o Tudy como atalho, nunca como ponto central. CTA: "Salva para aplicar" ou "Qual técnica você já usa? Comenta!".',
                    enabled: true
                },
                {
                    id: 'identificacao-dor',
                    name: 'Identificação com a Dor',
                    description: 'Muito conteúdo sem direção, procrastinação, véspera de prova — o caos do estudante',
                    weight: 15,
                    formats: ['carousel-premium', 'static'],
                    captionStyle: 'ESTRUTURA: 1) Abra com a emoção real (urgência, ansiedade — ex: "prova amanhã e você não começou"). 2) Entregue um plano concreto e acionável (3 passos para agora). 3) Seja útil MESMO sem o app — o conteúdo tem valor independente. 4) Tudy apenas como potencializador no fim. CTA: "Salva e manda pro grupo" ou "Comenta quanto tempo você tem".',
                    enabled: true
                }
            ],
            contentSchedule: {
                ...
                storiesPerWeek: 10,
                preferredDays: ['monday', 'wednesday', 'friday'],
                preferredTimes: ['08:00', '19:00'],
                autonomyMode: 'review',
                autoGenerateDay: 'sunday',
                autoGenerateTime: '20:00'
            },
            brandContext: `O Tudy é um app mobile de estudos com IA, feito para uso em smartphones. Ele transforma qualquer material ou dúvida em prática, explicação e progresso real direto no celular. Não é uma plataforma web, não é software para laptop/tablet e não deve ser apresentado como dashboard desktop. Não é apenas um gerador de resumos nem um app de quiz isolado — é um sistema operacional de estudos com IA.
...
Formatos prioritários: Reels (POV: manda foto da prova → Tudy resolve), Carrosséis premium (mini-aulas de método de estudo), Stories com quiz interativo.`,
            ...
            productService: `App mobile para smartphones, assistente de estudos com IA, que: (1) resolve questões por foto (exercício, prova, lousa) passo a passo; (2) cria estudos completos a partir de PDF, foto, áudio ou texto; (3) gera flashcards, quiz e questões discursivas automaticamente; (4) monta trilhas de preparação para provas com ordem e progresso; (5) tem comunidade com grupos de estudo e ranking; (6) aprende e adapta revisões ao desempenho do usuário.

Uso principal: estudante com o celular na mão, tirando foto, enviando material e estudando em sessões curtas pelo smartphone. Não representar como tablet, notebook, dashboard web ou plataforma desktop.`,
...
DEVICE RULE: Quando o produto aparecer, mostrar somente smartphone vertical. Não mostrar Tudy em tablets, laptops, monitores, dashboards desktop ou telas horizontais.
...
ALWAYS: Dark surfaces as base. Blue as primary interaction color. Progress and structure as visual motifs. Overlay UI elements must look editorial, not artificial.`
            ...
            brandKit: {
                ...
                voice: 'Inteligente, direta, encorajadora e focada. Entrega valor concreto antes de apresentar o app.',
                toneRules: [
                    'Ensine algo aplicável mesmo sem o Tudy.',
                    'Seja orientada à ação; nunca acadêmica fria, exagerada ou teatral.',
                    'Não apresente o Tudy como assunto central antes do CTA final.'
                ],
                copyArchetypes: [
                    'QUEBRE A CRENÇA: "RELER NÃO FIXA NADA NA MEMÓRIA"',
                    'REVELAÇÃO COM DADO: "SEU CÉREBRO ESQUECE 70% EM 24H"',
                    'TRANSFORMAÇÃO: "DE CADERNO BAGUNÇADO PARA TRILHA COMPLETA"',
                    'IDENTIFICAÇÃO: "VOCÊ ABRE O PDF E TRAVA — ISSO TEM SOLUÇÃO"',
                    'MÉTODO: "3 MINUTOS QUE VALEM MAIS QUE 3H DE LEITURA"'
                ],
                headlineExamples: [
                    'RELER NÃO FIXA NADA NA MEMÓRIA',
                    'SEU CÉREBRO ESQUECE 70% EM 24H',
                    'VOCÊ ABRE O PDF E TRAVA',
                    'MENOS HORAS, MAIS RESULTADO'
                ],
                narrativeStructure: {
                    description: 'dor → ciência → mito → método prático → Tudy como solução → CTA',
                    slideRoles: [
                        { position: '1', role: 'HOOK', rules: 'Identifique a dor real do estudante. Não mencione Tudy.' },
                        { position: 'middle', role: 'VALOR', rules: 'Ensine ciência, método ou prática ativa útil sem o app.' },
                        { position: 'last-1', role: 'SOLUÇÃO', rules: 'Revele o Tudy como a ferramenta que automatiza o método ensinado.' },
                        { position: 'last', role: 'CTA', rules: 'Convide a baixar grátis, testar agora ou salvar para usar.' }
                    ]
                },
                ctaRules: [
                    'Mencione o Tudy somente no final do arco narrativo.',
                    'Use CTA de app-download ou link na bio; nunca invente URLs.',
                    'Pode usar "Baixe grátis" ou "Comece agora" no CTA final.'
                ],
                forbiddenWords: ['O Tudy faz', 'Com o Tudy', 'Use o Tudy', 'Baixe o Tudy', 'app', 'aplicativo'],
                visualIdentity: {
                    photographyStyle: 'Estudantes reais concentrados em mesas, bibliotecas e cantos de estudo; smartphone vertical quando o produto aparecer.',
                    colorUsage: 'Fundos #0F1113 e #191C20; Azul Elétrico #2257F5 como acento e Violet #7C3AED para trilhas.',
                    typographyFeel: 'Space Grotesk bold para display; IBM Plex Sans para corpo. Cerebral e estruturada.',
                    imagePromptGuidelines: [
                        'Usar fundos escuros e azul elétrico como acento.',
                        'Nunca representar o produto em laptop, tablet, monitor ou dashboard desktop.',
                        'Não usar hologramas de IA, estética infantil, cyberpunk excessivo ou pessoas sorrindo sem contexto.',
                        'Não gerar texto, logos ou tipografia na imagem.'
                    ]
                },
                captionRules: [
                    'Abra identificando a dor do estudante na primeira ou segunda linha.',
                    'Continue o arco dos slides com valor prático antes do CTA.',
                    'Mantenha o tom direto, encorajador e preciso.'
                ],
                hashtagStrategy: 'Use hashtags específicas de estudo inteligente, prática ativa, vestibular e universidade; evite tags genéricas em excesso.',
                valuePillars: [
                ...
                doAlways: [
                    ...
                    'Conteúdo de valor primeiro — ensinar algo concreto antes de qualquer menção ao app',
                    'Tudy aparece como solução natural no fim, nunca como assunto central do post',
                    'Captions abrem identificando a dor do estudante (1ª ou 2ª pessoa) nas primeiras linhas'
                ],
                neverUse: [
    ...
    if (brandKey === 'elevepic') {
        return {
            brandKey: 'elevepic',
            description: 'ElevePic: Posicionamento visual estratégico para profissionais. Ajudamos a comunicar autoridade real — não encenada — através de fotografia e estética que cobra o que você vale.',
            brandContext: `A ElevePic não vende "foto bonita". Vende posicionamento visual estratégico. O produto é a percepção de autoridade, credibilidade e valor que a imagem do profissional gera no mercado.

O público não quer parecer uma pessoa diferente — quer parecer a melhor versão autêntica de si mesmo. Esse é o erro fatal de todo stock photo corporativo: ele cria distância emocional porque parece outra pessoa.

A transformação real é: o cliente continua sendo ele — mas agora a imagem dele comunica competência, confiança e posicionamento. Naturalidade elevada, não artificialidade premium.

Conceito central: "Nada artificial. Tudo naturalmente superior."`,

            targetAudience: `Profissionais liberais (consultores, coaches, advogados, médicos, arquitetos) que percebem que sua imagem visual não reflete o nível real de expertise que têm. Faixa 28-45 anos. Já têm resultados comprovados mas ainda parecem "mais um no mercado". Sentem que perdem clientes para concorrentes com posicionamento mais forte. Querem cobrar mais mas a imagem atual não sustenta o preço premium.`,

            productService: `Serviço de fotografia profissional estratégica para profissionais e marcas pessoais. Inclui: (1) consultoria de posicionamento visual — análise do que a imagem atual comunica vs. o que deveria comunicar; (2) direção de fotografia — iluminação, composição e enquadramento calculados para autoridade real; (3) direção de wardrobe — roupa que posiciona sem afastar (blazer + camiseta > terno completo); (4) edição editorial — realismo elevado, sem retoques excessivos que quebram credibilidade.`,

            contentStrategy: `4 linhas editoriais:
1. VOCÊ PERDE DINHEIRO COM FOTO RUIM: identificação com a dor — o custo invisível de uma imagem fraca no mercado
2. A DIFERENÇA QUE A IMAGEM FAZ: before/after de profissionais reais, transformações concretas de posicionamento
3. AUTORIDADE REAL vs. AUTORIDADE ENCENADA: educação sobre o que diferencia uma foto que vende de uma foto que impressiona mas não converte
4. BASTIDORES E PROCESSO: como é uma sessão ElevePic — humaniza a marca e quebra a barreira de entrada`,

            editorialPillars: [
                {
                    id: 'custo-imagem',
                    name: 'O Custo Invisível da Imagem Fraca',
                    description: 'Identificação com a dor: oportunidades perdidas por imagem que não posiciona corretamente',
                    weight: 30,
                    formats: ['carousel-html', 'carousel-premium'],
                    captionStyle: 'Identificação direta com a dor. Sem julgamento, mas honesto. CTA: salvar.',
                    enabled: true
                },
                {
                    id: 'transformacao',
                    name: 'Before & After de Posicionamento',
                    description: 'Comparativo visual de profissionais antes e depois — mostra o delta de percepção',
                    weight: 35,
                    formats: ['carousel-html', 'static'],
                    captionStyle: 'Resultado concreto, antes/depois emocional. CTA: conhecer o serviço.',
                    enabled: true
                },
                {
                    id: 'autoridade-real',
                    name: 'Autoridade Real vs. Autoridade Encenada',
                    description: 'Educação: o que torna uma foto estratégica vs. uma foto genérica de banco de imagens',
                    weight: 20,
                    formats: ['carousel-premium', 'carousel-html'],
                    captionStyle: 'Educativo, te faço ver o que você não via. CTA: salvar.',
                    enabled: true
                },
                {
                    id: 'bastidores',
                    name: 'Bastidores e Processo',
                    description: 'Como é uma sessão ElevePic — humaniza, quebra barreira de entrada, gera confiança',
                    weight: 15,
                    formats: ['static', 'story'],
                    captionStyle: 'Humanizado, direto, sem pompa. CTA: seguir.',
                    enabled: true
                }
            ],
            ...
            branding: {
                primaryColor: '#3542ed',
                secondaryColor: '#0c0e1c',
                style: 'Editorial premium com raízes no realismo. Fundos escuros (#0c0e1c) como base. Azul elétrico (#3542ed) como acento estratégico. Fotografia de pessoas: luz natural lateral, ambientes reais, naturalidade elevada — nunca encenada.',
                guidelines: `ElevePic Visual Brand Kit:
CORES: Fundo #0c0e1c (dominante), Azul Elétrico #3542ed (acentos/CTAs), Branco #F4F6FF (texto em fundo escuro), Cinza médio para textos secundários.
TIPOGRAFIA: Fontes sem-serif pesadas para headlines. Espaçamento controlado. Autoridade via peso, não via ornamento.
FOTOGRAFIA HUMANA: Luz lateral suave (janela real ou softbox difuso). Ambientes com estante de livros, escritório real, café com caráter. NUNCA skyline de cidade desfocado. Roupa: blazer + camiseta, camisa aberta, look consultor moderno — NUNCA terno completo com gravata. Expressão: calma, confiante, levemente aberta. NUNCA neutro robótico ou sorriso forçado. Postura: relaxada com peso numa perna, leve rotação de corpo, mãos soltas ou segurando algo — NUNCA simétrica travada. Pele: textura natural mantida, poros visíveis, imperfeições mínimas preservadas.
NUNCA: skyline, fundo preto dramático tipo capa de revista, terno completo, iluminação tipo holofote teatral, expressão dura/séria demais, pose institucional simétrica, pele super retocada.`
            },

            aiPreferences: {
                defaultAspectRatio: '4:5',
                style: 'Fotografia editorial de retrato autêntico. Luz natural lateral ou janela real — nunca iluminação dramática teatral. Ambientes reais com caráter: estante de livros, escritório funcional, café, não city-skylines ou estúdio genérico. Naturalidade elevada: a pessoa parece a melhor versão dela mesma, não outra pessoa.',
                tone: 'Direto, estratégico, sem rodeios. Autoridade que não precisa gritar. Nunca pomposo ou intimidador.',
                favoritePrompts: []
            },

            brandKit: {
                coreMessage: 'Sua imagem deve comunicar o que você realmente vale.',
                personality: 'Estratégica, direta, premium sem pompa, humana.',
                archetype: 'O Estrategista — credibilidade construída, não encenada.',
                voice: 'Direta, estratégica e premium sem pompa. Autoridade que não precisa gritar.',
                toneRules: [
                    'Mostre o custo concreto de uma imagem que não posiciona.',
                    'Evite jargão vazio e promessas de status sem evidência.',
                    'Prefira contraste claro entre amador e profissional.'
                ],
                copyArchetypes: [
                    'CONTRASTE: "AMADOR VS. PROFISSIONAL"',
                    'MITO: "NÃO É SORTE, É DESIGN"',
                    'STATUS: "PAREÇA CARO"'
                ],
                headlineExamples: ['PAREÇA CARO', 'NÃO É SORTE, É DESIGN', 'SUA FOTO COBRA POR VOCÊ'],
                visualIdentity: {
                    photographyStyle: 'Retratos editoriais autênticos, luz lateral suave e ambientes reais com caráter.',
                    colorUsage: 'Fundo #0c0e1c dominante, Azul Elétrico #3542ed como acento e branco suave para texto.',
                    typographyFeel: 'Sans-serif pesada, controlada, com autoridade via peso e espaço negativo.',
                    imagePromptGuidelines: [
                        'Cenas literais de posicionamento e autoridade; não usar metáforas abstratas.',
                        'Usar blazer com camiseta ou camisa aberta, expressão calma e postura relaxada.',
                        'Nunca usar skyline, terno com gravata, pose simétrica, pele excessivamente retocada ou luz teatral.'
                    ]
                },
                valuePillars: [
                    'Posicionamento visual estratégico (não "foto bonita")',
                    'Autoridade real vs. autoridade encenada',
                    'Naturalidade elevada — melhor versão autêntica, não personagem'
                ],
                editorialLines: [
                    'O Custo Invisível da Imagem Fraca',
                    'Before & After de Posicionamento',
                    'Autoridade Real vs. Autoridade Encenada',
                    'Bastidores e Processo'
                ],
                doAlways: [
                    'Usar fundo escuro (#0c0e1c) como base padrão',
                    'Luz lateral suave ou natural — cria profundidade sem drama excessivo',
                    'Roupa de posicionamento moderno: blazer + camiseta, camisa social aberta',
                    'Manter textura de pele natural — poros e micro-imperfeições preservam credibilidade',
                    'Postura relaxada com presença — leve rotação do corpo, mãos soltas',
                    'Expressão: calma confiante, leve abertura — nunca neutro robótico',
                    'Ambientes reais com caráter: estante de livros, escritório funcional, café'
                ],
                neverUse: [
                    'Skyline de cidade desfocado como fundo',
                    'Terno completo com gravata (parece "CEO de banco", afasta)',
                    'Iluminação cinematográfica exagerada tipo holofote',
                    'Pose simétrica travada com mãos perfeitamente posicionadas',
                    'Expressão neutra dura ou séria demais — parece robô',
                    'Pele super retocada sem nenhuma imperfeição — quebra realismo',
                    'Fundo preto puro tipo capa de revista',
                    'Ambientes perfeitos demais — estúdio genérico sem personalidade',
                    'Olhar para o lado dramático — sempre câmera direta',
                    'Estética "fake coach": luzes coloridas de estúdio, poses de braços cruzados olhando para longe'
                ],
                visualReferenceUrls: [],
                appUiReferenceUrls: [],
                referencePrompts: [],
                uiPatterns: [
                    'Fundo escuro #0c0e1c base',
                    'Azul elétrico #3542ed como acento',
                    'Tipografia pesada sem-serif',
...
ALWAYS: Anchor visuals to food, kitchen, eating, or nutrition context. Communicate utility, clarity, calm, competence.`
        ...
        brandKit: {
            ...
            voice: 'Direta, encorajadora e sem julgamento. Faz a pessoa se sentir capaz, nunca fiscalizada.',
            toneRules: [
                'Fale como uma amiga experiente que entrega clareza prática.',
                'Evite terrorismo nutricional, promessas vazias e linguagem genérica.',
                'Mostre prazer, facilidade e consistência em vez de restrição.'
            ],
            copyArchetypes: [
                'QUEBRA DE MITO: "Não é disciplina — é [X]"',
                'REVELAÇÃO: "O que [grupo] não te conta sobre [tema]"',
                'TRANSFORMAÇÃO: "De [estado ruim] para [estado desejado]"',
                'CONTRASTE: "[Coisa ruim] vs [solução boa da Fitswap]"',
                'PERMISSÃO: "Você pode [desejo] e ainda [resultado positivo]"',
                'URGÊNCIA SUAVE: "Enquanto você [ação errada], [consequência]"'
            ],
            headlineExamples: ['COMER BEM NÃO PRECISA SER SOFRIMENTO', 'A IA QUE DECIDE POR VOCÊ', 'GELADEIRA VAZIA, SOLUÇÃO CERTA'],
            visualIdentity: {
                photographyStyle: 'Fotografia editorial wellness-tech, humana e fotorrealista; cozinhas brasileiras práticas e luz natural suave.',
                colorUsage: 'Branco dominante (70-80%), Dark Gray #111827 para estrutura e Neon Lime #A6F000 somente como acento.',
                typographyFeel: 'Inter ou SF Pro, peso alto, clean e funcional.',
                imagePromptGuidelines: [
                    'A imagem deve ilustrar a cena específica da headline, não ser decoração genérica.',
                    'Usar pessoas reais, comida alcançável e espaço negativo limpo na metade inferior.',
                    'Evitar poses de influencer, comida gourmet, sombras pesadas, fundos escuros, texto e logos.'
                ]
            },
            valuePillars: [
...
function mergeBrandKit(profileBrandKit = {}, presetBrandKit = {}) {
    return {
        ...
        voice: normalizeText(profileBrandKit.voice) || normalizeText(presetBrandKit.voice),
        toneRules: useProfileRules(profileBrandKit.toneRules, presetBrandKit.toneRules),
        copyArchetypes: useProfileRules(profileBrandKit.copyArchetypes, presetBrandKit.copyArchetypes),
        headlineExamples: useProfileRules(profileBrandKit.headlineExamples, presetBrandKit.headlineExamples),
        narrativeStructure: mergeNarrativeStructure(profileBrandKit.narrativeStructure, presetBrandKit.narrativeStructure),
        ctaRules: useProfileRules(profileBrandKit.ctaRules, presetBrandKit.ctaRules),
        forbiddenWords: useProfileRules(profileBrandKit.forbiddenWords, presetBrandKit.forbiddenWords),
        forbiddenVisuals: useProfileRules(profileBrandKit.forbiddenVisuals, presetBrandKit.forbiddenVisuals),
        visualIdentity: mergeVisualIdentity(profileBrandKit.visualIdentity, presetBrandKit.visualIdentity),
        captionRules: useProfileRules(profileBrandKit.captionRules, presetBrandKit.captionRules),
        hashtagStrategy: normalizeText(profileBrandKit.hashtagStrategy) || normalizeText(presetBrandKit.hashtagStrategy)
    };
}
