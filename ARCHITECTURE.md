...
| Agendamento | UptimeRobot (5 min) + endpoint Basic Auth + leases Firestore |
| HTTP Client | Axios |
...
├── backend/
│   └── src/
│       ├── server.js              # Entry point — porta local 3011, timeout 5min
...
```
UptimeRobot (a cada 5 min) → `/internal/cron/tick` → schedulerService.runWeeklyAutoGeneration()
     ↓
...
Em produção, o scheduler interno fica desligado (`ENABLE_IN_PROCESS_SCHEDULER=false`). O UptimeRobot chama `GET|HEAD /internal/cron/tick` a cada cinco minutos usando HTTP Basic.

```text
UptimeRobot
  → autenticação CRON_USERNAME/CRON_PASSWORD
  → transação Firestore adquire lease de uptimerobot_tick
  → syncScheduledPosts() consulta jobs Upload-Post
  → runWeeklyAutoGeneration() retoma slots ausentes de forma idempotente
  → heartbeat/status terminal no Firestore
```

Ticks concorrentes não duplicam trabalho. Falhas de agendamento recebem `schedule_error`; não existe publicação local atrasada como fallback.
...
| Polling | schedulerService.js | Sincroniza status de posts externos a cada tick de 5 minutos |
...
ALLOWED_USER_UIDS=<uid-administrativo>
...
**5. Agendamento**
```
Monitor HTTP externo a cada cinco minutos
  → adquirir lease Firestore
  → sincronizar jobs já enviados ao Upload-Post
  → retomar geração semanal idempotente
  → registrar heartbeat e falhas auditáveis
```
...
O fluxo atual usa polling durável disparado pelo UptimeRobot a cada cinco minutos. Isso mantém sincronização auditável em uma hospedagem sem processo cron permanente e evita dois schedulers concorrentes.
