...
interface SpyPost {
    id: string; type: string; caption: string; imageUrl: string;
    likes: number; comments: number; videoViews: number | null;
    slideCount: number; timestamp: string; sourceUrl: string;
...
function IdeaPostCard({ idea, expanded, savingNote, onExpand, onDelete, onSaveNote, onUse, onRefresh }: {
    idea: SpyIdea;
    expanded: boolean;
    savingNote: boolean;
    onExpand: () => void;
    onDelete: () => void;
    onSaveNote: (idea: SpyIdea, note: string) => void;
    onUse: () => void;
    onRefresh: () => void;
}) {
    ...
    const renderMarkdown = (text: string) =>
        text
            .replace(/\*\*(.*?)\*\*/g, '<strong style="color:#a78bfa;font-weight:700;">$1</strong>')
            .replace(/\*(.*?)\*/g, '<em style="color:#cbd5e1;">$1</em>')
            .replace(/^- (.*)$/gm, '<li style="margin-left:16px;list-style:none;position:relative;"><span style="position:absolute;left:-12px;color:#a78bfa;">•</span>$1</li>')
            .replace(/\n\n/g, '<br/><br/>')
            .replace(/(?!>)\n(?!<)/g, '<br/>');
    ...
    return (
        <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '12px', overflow: 'hidden', display: 'flex', flexDirection: 'column', transition: 'all 0.2s' }}>
            {/* Thumbnail */}
            <div style={{ position: 'relative', aspectRatio: '1/1', background: '#0d0d10', flexShrink: 0 }}>
                {idea.post.imageUrl && !imgFailed ? (
                    <img
                        src={proxyImg(idea.post.imageUrl)}
                        alt="Ideia"
                    ...
                    <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'rgba(255,255,255,0.15)' }}>
                        <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.2"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><path d="m21 15-5-5L5 21"/></svg>
                    </div>
                )}

                {/* type badge */}
                {idea.post.type === 'video' && (
                    <div style={{ position: 'absolute', top: '8px', right: '8px', background: 'rgba(0,0,0,0.75)', borderRadius: '50%', width: '26px', height: '26px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <svg width="10" height="12" viewBox="0 0 10 12" fill="white"><path d="M0 0l10 6-10 6z"/></svg>
                    </div>
                )}
                {idea.post.type === 'carousel' && (
                    <div style={{ position: 'absolute', top: '8px', right: '8px', background: 'rgba(0,0,0,0.75)', borderRadius: '4px', padding: '2px 7px', fontSize: '0.65rem', color: 'white', fontWeight: 700 }}>
                        1/{idea.post.slideCount || 1}
                    </div>
                )}

                {/* Instagram link */}
                {idea.post.sourceUrl && (
                    <a href={idea.post.sourceUrl} target="_blank" rel="noopener noreferrer" title="Abrir no Instagram"
                ...
                {idea.post.ownerUsername && (
                    <div style={{ position: 'absolute', bottom: '36px', left: '8px', background: 'rgba(0,0,0,0.72)', borderRadius: '6px', padding: '2px 8px', fontSize: '0.68rem', color: 'white', fontWeight: 600 }}>
                        @{idea.post.ownerUsername}
                    </div>
                )}

                {/* stats bar */}
                <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '8px 12px', background: 'linear-gradient(to top, rgba(0,0,0,0.85), transparent)', display: 'flex', gap: '10px', color: 'white', fontSize: '0.72rem', fontWeight: 600 }}>
            ...
            <div style={{ padding: '14px', display: 'flex', flexDirection: 'column', gap: '10px', flex: 1 }}>
                ...
                {isProcessing && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '8px 10px', background: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.2)', borderRadius: '8px', fontSize: '0.75rem', color: '#f59e0b' }}>
                        <span style={{ animation: 'spin 1.5s linear infinite', display: 'inline-block' }}>⏳</span>
                        Gerando estratégia e conceito com IA...
                        <button onClick={onRefresh} title="Verificar se já terminou" style={{ marginLeft: 'auto', background: 'none', border: 'none', cursor: 'pointer', color: '#f59e0b', fontSize: '0.8rem', padding: 0 }}>🔄</button>
                    </div>
                )}

                {/* Expandable sections */}
                <div style={{ display: 'flex', gap: '6px' }}>
                    {['strategy', 'concept'].map(sec => {
                        ...
                        return (
                            <button
                                key={sec}
                                onClick={() => setActiveSection(isActive ? null : sec as 'strategy' | 'concept')}
                                disabled={!hasData}
                                style={{ flex: 1, padding: '6px 4px', fontSize: '0.7rem', fontWeight: 600, border: `1px solid ${isActive ? 'var(--accent-primary, #a78bfa)' : 'rgba(255,255,255,0.1)'}`, borderRadius: '7px', background: isActive ? 'rgba(167,139,250,0.15)' : 'rgba(255,255,255,0.03)', color: hasData ? (isActive ? '#a78bfa' : '#e2e8f0') : 'rgba(255,255,255,0.25)', cursor: hasData ? 'pointer' : 'default', transition: 'all 0.15s', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '3px' }}
                            >
                                {label}
                                {isActive ? ' ▲' : ' ▼'}
                            </button>
                ...
                {activeSection === 'strategy' && hasStrategy && (
                    <div style={{ background: 'rgba(167,139,250,0.06)', border: '1px solid rgba(167,139,250,0.15)', borderRadius: '8px', padding: '12px', fontSize: '0.78rem', color: '#e2e8f0', lineHeight: 1.65, maxHeight: '220px', overflowY: 'auto' }}
                        dangerouslySetInnerHTML={{ __html: renderMarkdown(idea.strategy!) }} />
                )}
                {activeSection === 'concept' && hasConcept && (
                    <div style={{ background: 'rgba(167,139,250,0.06)', border: '1px solid rgba(167,139,250,0.15)', borderRadius: '8px', padding: '12px', fontSize: '0.78rem', color: '#e2e8f0', lineHeight: 1.65, maxHeight: '220px', overflowY: 'auto' }}
                        dangerouslySetInnerHTML={{ __html: renderMarkdown(idea.concept!) }} />
                )}

                {/* My idea / note */}
                <div>
                    <label style={{ display: 'block', fontSize: '0.68rem', color: 'rgba(255,255,255,0.35)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '5px' }}>Minha Ideia / Adaptação</label>
                    <textarea
                        value={note}
                        onChange={e => setNote(e.target.value)}
                        onBlur={() => { if (note !== idea.note) onSaveNote(idea, note); }}
                        placeholder="Como você vai adaptar esse post para o seu negócio?"
                        rows={3}
                        style={{ width: '100%', resize: 'vertical', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', padding: '8px 10px', fontSize: '0.8rem', color: '#e2e8f0', lineHeight: 1.5, outline: 'none', fontFamily: 'inherit', boxSizing: 'border-box' }}
                        onFocus={e => { e.currentTarget.style.borderColor = '#a78bfa'; }}
                        onBlurCapture={e => { e.currentTarget.style.borderColor = 'rgba(255,255,255,0.1)'; }}
                    />
                    {savingNote && <div style={{ fontSize: '0.65rem', color: 'rgba(255,255,255,0.3)', marginTop: '3px' }}>Salvando...</div>}
                </div>
                ...
                <div style={{ display: 'flex', gap: '6px', marginTop: 'auto' }}>
                    <button
                        onClick={onUse}
                        style={{ flex: 1, padding: '8px', fontSize: '0.75rem', fontWeight: 700, background: 'linear-gradient(135deg, rgba(124,58,237,0.5), rgba(167,139,250,0.3))', border: '1px solid rgba(167,139,250,0.4)', borderRadius: '8px', color: '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '5px', transition: 'all 0.15s' }}
                        onMouseOver={e => e.currentTarget.style.background = 'linear-gradient(135deg, rgba(124,58,237,0.7), rgba(167,139,250,0.5))'}
                        onMouseOut={e => e.currentTarget.style.background = 'linear-gradient(135deg, rgba(124,58,237,0.5), rgba(167,139,250,0.3))'}
                    >
                        🚀 Usar no Gerador
                    </button>
                    <button
                        onClick={onDelete}
                        title="Remover"
                        style={{ padding: '8px 10px', background: 'rgba(239,68,68,0.07)', border: '1px solid rgba(239,68,68,0.2)', color: 'rgba(239,68,68,0.6)', cursor: 'pointer', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.15s' }}
                        onMouseOver={e => { (e.currentTarget as HTMLButtonElement).style.background = 'rgba(239,68,68,0.15)'; (e.currentTarget as HTMLButtonElement).style.color = '#ef4444'; }}
                        onMouseOut={e => { (e.currentTarget as HTMLButtonElement).style.background = 'rgba(239,68,68,0.07)'; (e.currentTarget as HTMLButtonElement).style.color = 'rgba(239,68,68,0.6)'; }}
                    >
...
function IdeasTab({ ideas, isLoading, expandedId, savingNoteId, onExpand, onDelete, onSaveNote, onUse, onRefresh, onGoSpy }: {
    ideas: SpyIdea[];
    isLoading: boolean;
    ...
    onSaveNote: (idea: SpyIdea, note: string) => void;
    onUse: (idea: SpyIdea) => void;
    onRefresh: (id: string) => void;
    onGoSpy: () => void;
}) {
    if (isLoading) {
        return (
            <div style={{ textAlign: 'center', padding: '4rem', color: '#a1a1aa', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '12px' }}>
    ...
    if (ideas.length === 0) {
        return (
            <div style={{ textAlign: 'center', padding: '5rem 2rem', color: '#a1a1aa' }}>
                <div style={{ fontSize: '52px', marginBottom: '16px' }}>💡</div>
                <h3 style={{ color: '#e2e8f0', marginBottom: '8px', fontSize: '1.1rem' }}>Nenhuma ideia salva ainda</h3>
                <p style={{ fontSize: '0.85rem', marginBottom: '24px', color: '#71717a' }}>
                    Espione contas no <strong style={{ color: '#a78bfa' }}>Espião de Competidores</strong> e clique em <strong style={{ color: '#f59e0b' }}>💡 Salvar Ideia</strong> nos posts que inspirarem você.
                </p>
                <button
                    onClick={onGoSpy}
                    style={{ padding: '10px 24px', background: 'linear-gradient(135deg, #7c3aed 0%, #a78bfa 100%)', border: 'none', borderRadius: '8px', color: '#fff', fontWeight: 600, cursor: 'pointer', fontSize: '0.9rem' }}
                >
                    🕵️ Ir para o Espião
                </button>
            </div>
    ...
    const pending = ideas.filter(i => !i.enriched && !i.strategy);
    return (
        <>
            {pending.length > 0 && (
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 14px', background: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.2)', borderRadius: '10px', marginBottom: '20px', fontSize: '0.82rem', color: '#f59e0b' }}>
                    <span style={{ animation: 'spin 1.5s linear infinite', display: 'inline-block' }}>⏳</span>
                    <span><strong>{pending.length}</strong> {pending.length === 1 ? 'ideia está sendo' : 'ideias estão sendo'} enriquecida{pending.length > 1 ? 's' : ''} com IA (estratégia + conceito). Recarregue em alguns segundos.</span>
                </div>
            )}
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: '1.5rem' }}>
                {ideas.map(idea => (
                    <IdeaPostCard
                        ...
                        savingNote={savingNoteId === idea.id}
                        onExpand={() => onExpand(idea.id)}
                        onDelete={() => onDelete(idea.id)}
                        onSaveNote={onSaveNote}
                        onUse={() => onUse(idea)}
                        onRefresh={() => onRefresh(idea.id)}
                    />
                ))}
            </div>
        </>
...
export default function LibraryPage() {
    ...
    // Ideas states
    const [ideas, setIdeas] = useState<SpyIdea[]>([]);
    const [isLoadingIdeas, setIsLoadingIdeas] = useState(false);
    const [expandedIdeaId, setExpandedIdeaId] = useState<string | null>(null);
    const [savingIdeaNote, setSavingIdeaNote] = useState<string | null>(null);
    ...
    useEffect(() => {
        if (selectedProfile && libraryTab === 'ideas') loadIdeas();
    }, [selectedProfile, libraryTab]);
    ...
    const handleSaveIdeaNote = async (idea: SpyIdea, note: string) => {
        setSavingIdeaNote(idea.id);
        try {
            await api.patch(`/api/competitors/ideas/${idea.id}`, { note });
            setIdeas(prev => prev.map(i => i.id === idea.id ? { ...i, note } : i));
        } catch {
            toast.error('Erro ao salvar nota.');
        } finally {
            setSavingIdeaNote(null);
        }
    };

    const handleUseIdea = (idea: SpyIdea) => {
        const text = idea.concept || idea.note || idea.post.caption || '';
    ...
    const handleRefreshIdea = async (ideaId: string) => {
        try {
            const res = await api.get(`/api/competitors/ideas?businessProfileId=${selectedProfile?.id}`);
            if (res.data.success) {
                const fresh = res.data.ideas.find((i: SpyIdea) => i.id === ideaId);
                if (fresh) setIdeas(prev => prev.map(i => i.id === ideaId ? fresh : i));
            }
        } catch { /* silent */ }
    };
                    ...
                    <>
                            ...
                            {(['posts', 'ideas'] as const).map(t => (
                                <button
                                    ...
                                    style={{
                                        ...
                                        background: libraryTab === t ? 'linear-gradient(135deg, #7c3aed 0%, #a78bfa 100%)' : 'transparent',
                                        color: libraryTab === t ? '#fff' : '#a1a1aa',
                                        position: 'relative' as const,
                                    }}
                                >
                                    {t === 'posts' ? '📚 Conteúdos' : '💡 Ideias Espionadas'}
                                    {t === 'ideas' && ideas.length > 0 && (
                                        <span style={{ position: 'absolute', top: '4px', right: '4px', background: '#f59e0b', color: '#000', borderRadius: '50%', width: '16px', height: '16px', fontSize: '0.6rem', fontWeight: 800, display: 'flex', alignItems: 'center', justifyContent: 'center', lineHeight: 1 }}>
                        ...
                        {libraryTab === 'ideas' && (
                            <IdeasTab
                                ...
                                expandedId={expandedIdeaId}
                                savingNoteId={savingIdeaNote}
                                onExpand={id => setExpandedIdeaId(prev => prev === id ? null : id)}
                                onDelete={handleDeleteIdea}
                                onSaveNote={handleSaveIdeaNote}
                                onUse={handleUseIdea}
                                onRefresh={handleRefreshIdea}
                                onGoSpy={() => router.push('/dashboard/competitors')}
                            />
                        )}
