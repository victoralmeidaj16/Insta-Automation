...
>
> **Atualização de estabilização — 03/08/2026:** este documento preserva o histórico dos commits de refatoração. O estado atual do código prevalece sobre observações antigas: Gemini de texto usa `gemini-2.5-flash`, o backend possui 49 testes aprovados, o frontend possui 2 testes aprovados, `tsc --noEmit` e o build Next passam. O scheduler de produção é acionado pelo UptimeRobot, não pelo `node-cron` interno.
...
- Modelos em uso: OpenAI `gpt-4o` / `gpt-4o-mini` e Gemini `gemini-2.5-flash` via REST. Consulte sempre as constantes no código para o identificador vigente.
...
- Histórico do Commit 0: as URLs Gemini foram centralizadas em `GEMINI_MODEL`/`GEMINI_URL`. O modelo foi posteriormente atualizado para `gemini-2.5-flash` após a retirada do 2.0 Flash.
